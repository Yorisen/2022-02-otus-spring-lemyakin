...
public class CommentsViewerTableWithBorder extends ViewerTableWithBorder implements CommentsViewer {
    private static final String[] HEADERS = {"id", "nickname", "content", "creation_timestamp"};
    ...
    void addCommentToTable(Object[][] table, int rowNumber, Comment comment) {
        String[] columns = {comment.getId(), comment.getNickname(), comment.getContent(),
                comment.getCreationTimestamp().toString()};
